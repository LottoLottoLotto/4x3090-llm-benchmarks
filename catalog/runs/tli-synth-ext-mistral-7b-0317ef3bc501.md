# `tli-synth-ext/mistral7b--no-spec-rerun/repetitive/c8`
...
```bash
'${ENGINE_ROOT_ALT}/vllm-env-025/bin/vllm' serve '${MODEL_ROOT}/Mistral-7B-v0.3' --host 127.0.0.1 --port 18241 --served-model-name bench --tensor-parallel-size 1 --language-model-only --max-model-len 8192 --max-num-seqs 8 --max-num-batched-tokens 8192 --gpu-memory-utilization 0.92 --attention-backend flash_attn --generation-config vllm
```
...
- `repo:llm-bench/results/daily-papers/2026-07-17/vllm-tli-cross-vocab-extended/mistral7b--no-spec-rerun.results.json`
